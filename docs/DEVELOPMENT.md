...
- 新增内容来源：实现 `src/workspace/types.ts` 中的 `WorkspaceProvider`，不要在 UI 中直接判断具体 Provider 来决定读写能力。
...
```text
为 TensorNote 增加笔记收藏功能。入口放在笔记标题右侧，收藏状态保存在浏览器本地；主页增加“已收藏”筛选。要求支持明暗主题和移动端，完成测试后提交、推送并发布 v0.1.1。
```
