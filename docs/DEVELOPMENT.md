...
- 新增或修改课程：编辑 `notes/**/*.md`。
- 修改目录和路由：编辑 `src/content/noteTree.ts` 与相关内容加载代码。
...
```text
为 TensorNote 增加笔记收藏功能。入口放在笔记标题右侧，收藏状态保存在浏览器本地；主页增加“已收藏”筛选。要求支持明暗主题和移动端，完成测试后提交、推送并发布 v0.1.0。
```
