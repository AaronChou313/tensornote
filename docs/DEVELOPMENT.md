...
v1.1.0 起，Desktop 使用 Tauri 2 并复用同一个 `src/`；v1.2.0 加入受限 Native Workspace 与 Native Git。先安装 Rust stable；macOS Homebrew 用户可运行 `brew install rust`。确认：
...
Native Workspace 改动还必须验证：路径穿越/符号链接拒绝、原子写与陈旧保存冲突、拖入 Markdown 的相对路径、真实临时 Git 仓库的 Status/Stage/Commit，以及 `pnpm build:web` 的 IPC 边界扫描。Static 产物中出现 `__TAURI_INTERNALS__`、`native_workspace_` 或 `native_git_` 会直接失败。
