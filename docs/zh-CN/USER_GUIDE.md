...
收件人打开链接即可阅读仓库默认分支。需要指定分支可在末尾添加 `?ref=main`（其他分支名需 URL 编码）。已有版本支持这个地址；下一补丁版本的“分享 Workspace”还提供“知识库最新内容链接”复制入口。需要保存可复现的阅读版本时，使用同一窗口中的固定 Revision 链接。分享不会授予写权限或自动信任仓库代码，也不包含 Jupyter 或 GitHub Token。
