...
适用版本：1.16.0。按“下载 → 打开 → 按需配置 → 日常使用”组织。只阅读、编辑 Markdown 不需要 Jupyter；执行 Python 才需要计算环境。TensorNote 不托管你的知识库或公共算力。
...
1. 从 [GitHub Releases](https://github.com/AaronChou313/tensornote/releases) 下载 `TensorNote-local-web-1.16.0.tar.gz`，解压。
...
`TensorNote-web-1.16.0.tar.gz` 是用于 `/tensornote/` 路径部署的 Static Web 包；它不是本地 Web 启动包。GitHub 自动提供的 “Source code” 则是开发者源码。
...
从同版本 Release 下载 `TensorNote-agent-skill-1.16.0.tar.gz`，解压后把 `tensornote-knowledge-workspace/SKILL.md` 连同所在目录交给支持 Skill 的智能体；不能只复制标题或丢弃引用文件。明确指定你的知识库目录，不要让它误改 TensorNote 应用源码目录。
