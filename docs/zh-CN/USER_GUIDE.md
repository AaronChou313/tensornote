...
适用版本：1.16.1。按“下载 → 打开 → 按需配置 → 日常使用”组织。只阅读、编辑 Markdown 不需要 Jupyter；执行 Python 才需要计算环境。TensorNote 不托管你的知识库或公共算力。
...
1. 从 [GitHub Releases](https://github.com/AaronChou313/tensornote/releases) 下载 `TensorNote-local-web-1.16.1.tar.gz`，解压。
...
`TensorNote-web-1.16.1.tar.gz` 是用于 `/tensornote/` 路径部署的 Static Web 包；它不是本地 Web 启动包。GitHub 自动提供的 “Source code” 则是开发者源码。
...
“最近打开”标题右侧可清空全部记录，每一项末尾也可单独移除。该操作只清除当前设备上的快捷入口，不会删除知识库文件、GitHub 仓库或执行信任记录。重新打开未指定固定 Revision 的 GitHub 链接时，TensorNote 会重新解析该分支的最新提交；已带完整 commit Revision 的分享链接仍固定显示对应版本。
...
- Overview 显示知识库入口和统计；左侧 Files 严格保持文件系统的目录层级与目录名，文件夹优先，并分别按真实目录名和文件名自然排序。Frontmatter 的 `section` 不会改写 Files 目录。折叠分区和隐藏整个侧栏是不同操作。
...
### 自定义 Overview（v1.6.2 起）

在知识库根目录放置 `README.md`，Overview 会直接显示其 Markdown 正文，支持相对图片和指向库内笔记的链接，无需通过 TensorNote 配置。若同时存在 `OVERVIEW.md`，优先使用后者，方便将 GitHub 项目介绍与阅读首页分开。文件名不区分大小写；即使 `content.root` 指向子目录，首页文件仍放在知识库根目录。没有这两个文件时继续显示自动生成的笔记入口。

顶部保留知识库名称和统计。简短介绍可在已有 `tensornote.yaml` 的 `workspace.description` 中修改。首页中的代码块只展示，不会自动执行。外部编辑完成后，刷新或重新打开知识库即可载入最新内容。

### 目录、大纲与双栏滚动（v1.6.2 起）

没有 `section` 的普通 Markdown 使用实际目录名称；`images` 等不含 Markdown 的目录显示为文件夹，图片通过正文引用显示。宽屏阅读时右侧大纲保持可见，长大纲独立滚动；窄屏可用顶部上下文栏按钮打开目录。双栏预览默认开启“同步滚动”，按标题位置在两侧换算滚动距离，避免累计像素偏差；无标题时按整体进度对齐。标题之间为近似对齐，可关闭同步分别浏览。

### 一条链接分享公开 GitHub 知识库

将 `OWNER/REPO` 替换为公开仓库的所有者和名称：

```text
https://aaronchou313.github.io/tensornote/#/open/github/OWNER/REPO
```

收件人打开链接即可阅读仓库默认分支。需要指定分支可在末尾添加 `?ref=main`（其他分支名需 URL 编码）。已有版本支持这个地址；v1.6.2 的“分享 Workspace”还提供“知识库最新内容链接”复制入口。需要保存可复现的阅读版本时，使用同一窗口中的固定 Revision 链接。分享不会授予写权限或自动信任仓库代码，也不包含 Jupyter 或 GitHub Token。
...
从同版本 Release 下载 `TensorNote-agent-skill-1.16.1.tar.gz`，解压后把 `tensornote-knowledge-workspace/SKILL.md` 连同所在目录交给支持 Skill 的智能体；不能只复制标题或丢弃引用文件。明确指定你的知识库目录，不要让它误改 TensorNote 应用源码目录。
...
通过固定 Revision 的 GitHub 分享链接打开公开知识库后，实验页会检查 Binder 环境入口。可用时点击“打开 Binder”，首次构建可能等待数分钟；环境与文件是临时的，请下载需要保留的产物。链接固定到完整 commit，不会携带 Token 或本机设置。未配置 Binder 时仍可完整阅读，并可改用桌面版。
