...
清单推荐命名为 `tensornote.experiment.yaml`，并遵循 [JSON Schema](../schemas/tensornote-experiment-v1.schema.json)。顶层必须包含 `schemaVersion: 1`、`experiment`、`environments`、`presets`、`defaultPreset` 和 `steps`；可包含强类型 `parameters`、资源提示 `resources` 与 `artifacts`。
