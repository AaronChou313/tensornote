...
export function ExperimentCard({ source, noteId }: { source: string; noteId?: string }) {
  ...
  const binder = session ? createBinderExperimentTarget(session, experiment) : null
  return <Link className="experiment-card" to={`/experiments/${encodeURIComponent(experiment.key)}`}>
    ...
    <span className={`experiment-card__status ${hasErrors ? 'is-error' : ''}`}>{hasErrors ? '需修复' : binder?.configured ? 'Binder 可用' : experiment.readOnly ? '只读' : '已索引'}</span>
