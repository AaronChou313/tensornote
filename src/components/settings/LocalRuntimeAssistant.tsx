...
export default function LocalRuntimeAssistant({ onRequestExistingJupyter }: { onRequestExistingJupyter?: () => void }) {
  ...
  const lastLocalEnvironmentId = useComputeStore((state) => state.lastLocalEnvironmentId)
  const setLastLocalEnvironment = useComputeStore((state) => state.setLastLocalEnvironment)
  ...
  const [selectedEnvironmentId, setSelectedEnvironmentId] = useState(lastLocalEnvironmentId)
  ...
  const discover = useCallback(async () => {
    ...
    try {
      ...
      setDiscovery(next)
      setServers(owned)
      const selected = next.environments.some((item) => item.id === selectedEnvironmentId) ? selectedEnvironmentId : next.environments.find((item) => item.jupyterInstalled)?.id || next.environments[0]?.id || ''
      setSelectedEnvironmentId(selected)
      if (selected) setLastLocalEnvironment(selected)
      setRequest((current) => {
  ...
  }, [adapter, selectedEnvironmentId, setLastLocalEnvironment, workspaceId])
  ...
  const selectedEnvironmentItem = environmentItems.find((item) => item.environment.id === selectedEnvironmentId)
  ...
  const loadServerLogs = async (serverId: string) => {
  ...
  return <section className="local-runtime-assistant" aria-label="本地 Python 环境">
    ...
    <div className="local-runtime-environment-list" role="radiogroup" aria-label="Python 环境">{environmentItems.map((item) => { const { environment } = item; const selected = selectedEnvironmentId === environment.id; return <Fragment key={environment.id}><button role="radio" aria-checked={selected} aria-expanded={selected} className={selected ? 'is-selected' : ''} onClick={() => { setSelectedEnvironmentId(environment.id); setLastLocalEnvironment(environment.id) }}><span className="local-runtime-environment-list__icon"><Flask size={17} /></span><span><strong>{environment.name}</strong><small>{item.managerLabel} · Python {environment.pythonVersion} · {environment.managed ? 'TensorNote 管理' : '外部环境'}</small><code title={item.path}>{item.path}</code><small>{environment.jupyterInstalled ? 'Jupyter 就绪' : '缺少 Jupyter 支持'} · {item.kernels.length} 个 Kernel</small></span><em data-status={item.status}>{item.statusLabel}</em></button>{selected && <div className="local-runtime-environment-details"><header><strong>Jupyter Kernel</strong><small>{item.kernels.length ? `${item.kernels.length} 个已注册 Kernel` : '尚未注册 Kernel'}</small></header>{item.kernels.length > 0 ? <ul>{item.kernels.map((kernel, index) => { const current = item.currentKernelName === kernel.name; const defaultKernel = environment.kernelName === kernel.name || (!environment.kernelName && index === 0); return <li key={kernel.name}><span className={`kernel-dot kernel-dot--${current ? 'busy' : 'idle'}`} /><span><strong>{kernel.displayName}</strong><small>{kernel.name} · {kernel.language}</small></span><em>{current ? '当前 Kernel' : defaultKernel ? '默认 Kernel' : ''}</em></li> })}</ul> : <p>该 Python 环境还没有可用的 Jupyter Kernel。</p>}{item.server && <details className="local-runtime-server-details"><summary><span><CheckCircle size={15} weight="fill" /><strong>Jupyter Server</strong></span><small>{item.server.status === 'running' ? '运行中' : item.server.status}</small></summary><div><code>{item.server.url}</code><span><Button variant="ghost" size="sm" onClick={() => void loadServerLogs(item.server!.id)}>刷新日志</Button><Button variant="danger" size="sm" onClick={() => void stopServer(item.server!.id)}><Stop size={13} weight="fill" />停止</Button></span></div>{logs[item.server.id] && <pre>{logs[item.server.id].map((line) => `[${line.stream}] ${line.text}`).join('\n') || '暂无日志'}</pre>}</details>}</div>}</Fragment> })}{!discovering && discovery.environments.length === 0 && <div className="local-runtime-empty"><WarningCircle size={18} /><span><strong>没有找到 Python 环境</strong><small>可先检查下方管理器，然后新建独立环境。</small></span></div>}</div>
    <div className="local-runtime-launch"><div><strong>{selectedEnvironment?.jupyterInstalled ? `使用 ${selectedEnvironment.name}` : '选择可启动的环境'}</strong><small>{selectedEnvironment?.jupyterInstalled ? 'Server 仅绑定 127.0.0.1；Token 只保留在当前应用会话。' : '缺少 Jupyter 的外部环境可在确认后补齐 Notebook 支持；TensorNote 不会删除外部环境。'}</small></div>{selectedEnvironment && !selectedEnvironment.managed && !selectedEnvironment.jupyterInstalled ? <Button variant="secondary" size="sm" onClick={() => void planJupyterSupport()}>安装 Jupyter 支持</Button> : <Button size="sm" onClick={() => void startServer()} disabled={!selectedEnvironment?.jupyterInstalled}><Play size={14} weight="fill" />{selectedEnvironmentItem?.server ? '连接此 Server' : '启动并连接'}</Button>}</div>
