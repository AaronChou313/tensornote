...
export default function LocalRuntimeAssistant({ onRequestExistingJupyter }: { onRequestExistingJupyter?: () => void }) {
  ...
  return <section className="local-runtime-assistant" aria-label="本地 Python 环境">
    ...
    {plan && <div className="local-runtime-plan"><header><div><small>创建前确认</small><strong>{plan.name} · Python {plan.pythonVersion}</strong></div><span>{managerLabel(plan.manager)}</span></header><p className="local-runtime-plan__target"><FolderOpen size={14} />目标位置 <code>{plan.targetPath || plan.targetLabel}</code></p>{plan.managerExecutablePath && <p className="local-runtime-plan__target"><TerminalWindow size={14} />管理器 <code>{plan.managerExecutablePath}</code></p>}<p className="local-runtime-plan__target"><Flask size={14} />Kernel <code>{plan.kernelName}</code></p><ol>{plan.steps.map((step) => <li key={step}>{step}</li>)}</ol><label><span>输入 <code>{plan.confirmation}</code></span><input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></label><div><Button size="sm" onClick={() => void applyPlan()} disabled={confirmation !== plan.confirmation}>确认并创建</Button><Button variant="ghost" size="sm" onClick={() => setPlan(null)}>取消</Button></div></div>}
