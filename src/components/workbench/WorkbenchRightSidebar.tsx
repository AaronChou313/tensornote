...
export function WorkbenchRightSidebar({ noteId }: { noteId?: string }) {
  ...
  return <aside className="workbench-right-sidebar" aria-label="当前焦点笔记的上下文栏">
    <header><div className="workbench-side-tabs"><button className={rightView === 'properties' ? 'is-active' : ''} onClick={() => setRightView('properties')} aria-label="当前笔记属性"><SlidersHorizontal size={16} /></button><button className={rightView === 'outline' ? 'is-active' : ''} onClick={() => setRightView('outline')} aria-label="当前笔记目录"><List size={16} /></button><button className={rightView === 'backlinks' ? 'is-active' : ''} onClick={() => setRightView('backlinks')} aria-label="当前笔记反向链接"><LinkSimple size={16} /></button><button className={rightView === 'graph' ? 'is-active' : ''} onClick={() => setRightView('graph')} aria-label="当前笔记图谱"><ShareNetwork size={16} /></button></div><button onClick={() => setSidebar('right', false)} aria-label="关闭右侧栏"><X size={16} /></button></header>
