  ...
  return (
    <button className="lab-card sidecar-card group" onClick={() => {
      ...
      <span className="min-w-0 flex-1 text-left">
        <span className="block text-[11px] font-medium text-[var(--accent)]">{isJupyter ? 'JUPYTER SIDECAR' : 'DERIVATION SIDECAR'}</span>
        <span className="mt-1 block text-base font-semibold tracking-[-0.01em] text-[var(--ink)]">{sidecar.title}</span>
        <span className="mt-1 block text-xs text-[var(--faint)]">{isJupyter ? `${sidecar.cells.length} 个 Python Cell，共享当前 Kernel` : '在侧栏中展开完整推导'}</span>
      </span>
