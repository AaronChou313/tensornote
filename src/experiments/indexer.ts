...
export async function indexExperiments(provider: WorkspaceProvider, notes: Note[]): Promise<IndexedExperiment[]> {
  ...
  for (const note of notes) for (const reference of note.experimentReferences) {
    ...
    try {
      ...
      indexed.push({ key, noteId: note.id, notePath: note.path, manifestPath, requestedPreset: reference.preset, manifest: result.manifest, diagnostics, readOnly: result.readOnly || diagnostics.some((item) => item.severity === 'error') })
    } catch (reason) {
