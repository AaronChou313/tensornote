...
function EditorSettings() {
  ...
  return <section className="settings-panel"><header><span>Authoring</span><h2>编辑器</h2><p>这些选项会应用到之后打开的笔记。</p></header><div className="settings-group"><SettingRow title="默认打开模式" description="选择阅读或 Markdown 源码编辑。"><select value={mode} onChange={(event) => setMode(event.target.value as EditorMode)}><option value="read">阅读</option><option value="edit">编辑</option></select></SettingRow><SettingRow title="显示行号" description="同时控制折叠标记栏。"><label className="settings-switch"><input type="checkbox" checked={lineNumbers} onChange={(event) => setLineNumbers(event.target.checked)} /><i /></label></SettingRow><SettingRow title="长行自动换行" description="关闭后可横向滚动查看源码。"><label className="settings-switch"><input type="checkbox" checked={wordWrap} onChange={(event) => setWordWrap(event.target.checked)} /><i /></label></SettingRow></div></section>
}
