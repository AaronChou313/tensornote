...
export function NotePage() {
  ...
  const { panes, activePane, secondaryOpen, secondaryPosition, rightSidebar, headingRequest, setActivePane } = useWorkbenchStore()

  const paneRoot = useRef<HTMLDivElement>(null)
  useEffect(() => {
    if (!headingRequest) return
    const pane = paneRoot.current?.querySelector(`[data-pane="${headingRequest.pane}"]`)
    if (pane) scrollToHeading(pane, headingRequest.id)
  }, [headingRequest])
  ...
  const showPane = (pane: 'main' | 'secondary', paneNote?: Note) => <section className="workbench-pane" data-pane={pane} data-active={activePane === pane} onMouseDown={() => setActivePane(pane)} onFocusCapture={() => setActivePane(pane)}>
  ...
  return <div ref={paneRoot} className={`workbench-panes ${secondaryOpen ? 'workbench-panes--split' : ''} ${secondaryPosition === 'left' ? 'workbench-panes--secondary-left' : ''} ${rightSidebar ? 'workbench-panes--context' : ''}`}>
