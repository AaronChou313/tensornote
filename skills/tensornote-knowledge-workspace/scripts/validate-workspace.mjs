...
export async function validateWorkspace(rootArg, { strict = false } = {}) {
  ...
  try {
    ...
    for (const doc of documents) {
      ...
      const sidecarOpening = /^:::tensornote\{([^}]*)\}\s*$/gm
      for (const match of doc.body.matchAll(sidecarOpening)) {
        const attrs = Object.fromEntries([...match[1].matchAll(/([\w-]+)="([^"]*)"/g)].map((item) => [item[1], item[2]]))
        if (!['derivation', 'jupyter'].includes(attrs.type)) report('error', 'sidecar-type', doc.path, 'Sidecar type must be derivation or jupyter')
        if (!kebab(attrs.id ?? '')) report('error', 'sidecar-id', doc.path, 'Sidecar id must use lowercase kebab-case')
        else if (sidecarIds.has(attrs.id)) report('error', 'sidecar-id-duplicate', doc.path, 'Sidecar id must be unique within the note')
        else sidecarIds.add(attrs.id)
        if (!attrs.title?.trim()) report('warning', 'sidecar-title', doc.path, 'Sidecar should have a useful title')
