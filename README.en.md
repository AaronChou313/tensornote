...
[Try online](https://aaronchou313.github.io/tensornote/) · [Download](https://github.com/AaronChou313/tensornote/releases) · [User guide](docs/en/USER_GUIDE.md) · [Release notes](docs/releases/v1.18.0.en.md)
...
**Local Web:** download `TensorNote-local-web-1.18.0.tar.gz` from [Releases](https://github.com/AaronChou313/tensornote/releases), extract it and run in that directory:
...
`TensorNote-web-1.18.0.tar.gz` is the Static Web archive for deployment at `/tensornote/`. GitHub's automatic Source code archives are for development. Local browser users should choose **local-web**.

## Configure experiments and Git only when needed

- **Online compute:** Settings → Compute & Jupyter → Remote runtime, then add Generic Jupyter, JupyterHub or BinderHub from the connection list. Successful diagnostics discover kernels automatically. Use your own HTTPS endpoint, identity and kernel. The service must allow the TensorNote Origin and WebSockets. A notebook sharing link is not a server endpoint. See the [compute walkthrough](docs/en/USER_GUIDE.md#compute).
...
- **Desktop experiments:** open **Local runtime**, then select or create a Conda/uv/venv environment from the environment list, then start and connect. Conda or uv can create Python 3.10–3.14 and the full target path is shown before confirmation. External environments can receive reviewed Jupyter support; server state, kernels, logs and stop controls stay in the environment details. The Experiment Environment tab can install selected `requirements*.txt` files into the current environment through a digest-bound plan.
...
Download the matching `TensorNote-agent-skill-1.18.0.tar.gz`. Give the agent the extracted `SKILL.md` **and all referenced files**, and explicitly identify your knowledge folder. It can generate and update notes, maintain links/assets, check prerequisites and run the bundled validator.
