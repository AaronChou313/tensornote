...
[Try online](https://aaronchou313.github.io/tensornote/) · [Download](https://github.com/AaronChou313/tensornote/releases) · [User guide](docs/en/USER_GUIDE.md) · [Release notes](docs/releases/v1.17.0.en.md)
...
**Local Web:** download `TensorNote-local-web-1.17.0.tar.gz` from [Releases](https://github.com/AaronChou313/tensornote/releases), extract it and run in that directory:
...
`TensorNote-web-1.17.0.tar.gz` is the Static Web archive for deployment at `/tensornote/`. GitHub's automatic Source code archives are for development. Local browser users should choose **local-web**.
...
- **Desktop experiments:** choose **Local runtime → Convenient connection**, detect or create a Conda/uv/venv environment, then start and connect. Conda or uv can create Python 3.11 and the full target path is shown before confirmation. The Experiment Environment tab can install selected `requirements*.txt` files into the current environment through a digest-bound plan.
...
Download the matching `TensorNote-agent-skill-1.17.0.tar.gz`. Give the agent the extracted `SKILL.md` **and all referenced files**, and explicitly identify your knowledge folder. It can generate and update notes, maintain links/assets, check prerequisites and run the bundled validator.
