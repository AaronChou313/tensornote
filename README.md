...
[在线体验](https://aaronchou313.github.io/tensornote/) · [下载应用](https://github.com/AaronChou313/tensornote/releases) · [使用说明](docs/zh-CN/USER_GUIDE.md) · [版本说明](docs/releases/v1.16.1.md)
...
**本地 Web：** 在 [Releases](https://github.com/AaronChou313/tensornote/releases) 下载 `TensorNote-local-web-1.16.1.tar.gz`，解压，在该目录运行：
...
`TensorNote-web-1.16.1.tar.gz` 是供 `/tensornote/` 路径部署的 Static Web 包；GitHub 自动生成的 Source code 是开发源码。普通本地 Web 用户选择 **local-web** 包。
...
v1.16.0 Project Experiment 入口（公开 Happy-LLM 试点，固定 commit；[来源记录](docs/images/v1.16.0/PROVENANCE.md)）：
...
下载同版本 `TensorNote-agent-skill-1.16.1.tar.gz`，把解压目录中的 `SKILL.md` **和全部引用文件**交给智能体，并明确指定你的知识库目录。它可按现有规范生成、更新、整理链接与附件、检查前置关系并运行校验器。
